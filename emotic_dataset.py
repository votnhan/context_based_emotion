...
class Emotic_CSVDataset(Dataset):
  ...
  def convert_to_rgb_image(self, image):
    arr = np.array(image)
    if len(arr.shape) < 3:
      exp_arr = np.expand_dims(arr, axis=2)
      rgb_arr = np.repeat(exp_arr, 3, axis=2)
    elif arr.shape[2] == 3:
      return image
    elif arr.shape[2] > 3:
      rgb_arr = arr[:, :, :3]
  ...
  def __getitem__(self, index):
    ...
    image_context = cv2.imread(os.path.join(self.data_src, row['Folder'], 
                      row['Filename']))
    rgb_img = cv2.cvtColor(image_context, cv2.COLOR_BGR2RGB)
    image_context = self.convert_to_rgb_image(rgb_img)
