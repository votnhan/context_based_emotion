...
def infer(context_norm, body_norm, ind2cat, ind2vad, device, models, 
            thresholds=None, image_context_path=None, image_context=None, 
            image_body=None, bbox=None, to_print=True, topk=4):
  ...
  '''
  image_context, image_body = process_images(context_norm, body_norm, 
                                image_context_path=image_context_path, 
                                image_context=image_context, 
                                image_body=image_body, bbox=bbox)
  ...
  with torch.no_grad():
    ...
    if thresholds is not None:
      ...
      desc_idx = torch.argsort(pred_cat, descending=True)
      chosen_idx = desc_idx[:topk]
      for i in range(chosen_idx.size(0)):
        cat_emotions.append(ind2cat[chosen_idx[i].item()])

    top_emotion_idx = torch.argmax(pred_cat).item()
  ...
  if to_print == True:
    ...
    print('Top emotion: {}'.format(ind2cat[top_emotion_idx]))
